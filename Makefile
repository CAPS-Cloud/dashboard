...
MAIN_PACKAGE = github.com/CAPS-Cloud/dashboard/src/app/backend
...
RELEASE_IMAGE_NAMES += $(foreach arch, $(ARCHITECTURES), ${REGISTRY_URL}/$(RELEASE_IMAGE)-$(arch):$(RELEASE_VERSION))
RELEASE_IMAGE_NAMES_LATEST += $(foreach arch, $(ARCHITECTURES), ${REGISTRY_URL}/$(RELEASE_IMAGE)-$(arch):latest)
HEAD_IMAGE = kubernetesdashboarddev/dashboard
HEAD_VERSION = latest
HEAD_IMAGE_NAMES += $(foreach arch, $(ARCHITECTURES), ${REGISTRY_URL}/$(HEAD_IMAGE)-$(arch):$(HEAD_VERSION))
ARCHITECTURES = amd64
